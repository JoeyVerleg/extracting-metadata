...
driver = webdriver.Chrome(executable_path=r'/home/joey/Desktop/fingerprinting/tool/chromedriver', options=chrome_options)
...
for url in urls:
    for page_load in range(0, max_page_loads):
        try:
            ...
            p_process = subprocess.Popen(['tcpdump', '-U', '-w ' + str(id) + '-' + str(page_load) + '.pcap'])
            time.sleep(SETUP_DELAY)
            driver.get(url)
